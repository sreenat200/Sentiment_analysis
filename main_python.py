...
nltk.data.path.append(nltk_data_path)  # Ensure the punkt tokenizer is available
...
def split_into_sentences(text, language="en"):
    try:
        ...
        if language == "en":
            ...
            # NLTK can sometimes handle Malayalam reasonably well
            sentences = sent_tokenize(text)
...
def create_zip_archive(audio_path, raw_transcription, ml_translation, pdf_report, 
    ...
    # Create base filename with user input and lead score
    base_filename = f"{user_filename}_L{lead_score}_I{intent_score}_{timestamp}"
    zip_filename = f"analysis_results/{base_filename}.zip"
    
    # Use WAV file if available (from transcription results)
    wav_path = None
    if audio_path.lower().endswith('.wav'):
        wav_path = audio_path
    else:
        # Try to find the converted WAV file in temp directory
        temp_dir = os.path.join(tempfile.gettempdir(), "whisper_temp")
    ...
    with zipfile.ZipFile(zip_filename, 'w') as zipf:
        # Add audio file (prefer WAV if available)
        if wav_path and os.path.exists(wav_path):
            audio_ext = '.wav'
            new_audio_name = f"{base_filename}{audio_ext}"
            zipf.write(wav_path, arcname=new_audio_name)
        elif os.path.exists(audio_path):  # Fallback to original file
...
def analyze_text(text, language="en"):
    ...
    for i, sentence in enumerate(sentences):
        ...
        # Get sentiment analysis as fallback if needed
        sentiment_result = analyze_sentiment_batch([sentence])[0] if sentence.strip() else {
            "label": "neutral",
            "score": 0.5
        }
        
        # Use intent-based sentiment if available, otherwise use sentiment analysis
...
def main_analysis_workflow():
    ...
    try:
        ...
        # Get user input for filename and lead score
