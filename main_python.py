...
class MalayalamTranscriptionPipeline:
    ...
    def transcribe_audio(self, audio_path):
        try:
            ...
            # Clean the full transcription
            cleaned_text = self.clean_transcription(
                full_text,
                remove_duplicate_sentences=True,
                remove_duplicate_words=False
            )

            return {
                "raw_transcription": cleaned_text,
...
def split_into_sentences(text, language="en"):
    try:
        if not text.strip():
            print("Empty text provided, returning empty list.")
            return []

        print(f"Attempting to split text: {text[:100]}...")  # Log first 100 chars
        # Try NLTK for English
        if language == "en":
            try:
                sentences = sent_tokenize(text)
                if len(sentences) > 1:
                    print(f"NLTK returned {len(sentences)} sentences.")
                    return [s.strip() for s in sentences if s.strip()]
                print("NLTK returned single sentence, trying fallback.")
        ...
        if language == "ml":
            try:
                ...
                print(f"Indic NLP returned {len(sentences)} sentences.")
                return [s.strip() for s in sentences if s.strip()]
            except Exception as indic_error:
                print(f"Indic NLP Malayalam tokenizer failed: {str(indic_error)}")

        # Fallback: Custom regex-based splitting
        print("Using fallback regex sentence splitting")
        sentence_endings = re.compile(r'(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?|\!)\s')
        sentences = sentence_endings.split(text)
        sentences = [s.strip() for s in sentences if s.strip()]
        if not sentences:
            print("Regex splitting failed, returning original text.")
            return [text] if text.strip() else []
        print(f"Regex splitting returned {len(sentences)} sentences.")
        return sentences
...
def analyze_text(text, language="en"):
    try:
        ...
        if not sentences:
            print(f"No sentences found for {language} text analysis.")
            return []
        ...
        print(f"Text analysis failed for {language}: {str(e)}")
        return []
...
def save_analysis_to_csv(analysis, filename_prefix):
    try:
        ...
        print(f"Failed to save analysis to CSV: {str(e)}")
        return None
...
def csv_to_pdf(csv_path, title, output_pdf_path):
    ...
    try:
        ...
        print(f"Failed to convert CSV to PDF: {str(e)}")
        return None
...
def combine_pdfs(pdf_paths, output_path):
    ...
    try:
        ...
        print(f"Failed to combine PDFs: {str(e)}")
        return None
...
def generate_analysis_pdf(en_analysis, ml_analysis, comparison, filename_prefix):
    ...
    try:
        ...
        print(f"Failed to generate analysis PDF: {str(e)}")
        return None
...
def compare_analyses(en_analysis, ml_analysis):
    try:
        ...
        print(f"Failed to compare analyses: {str(e)}")
        return []
...
def print_analysis_summary(analysis, title):
    try:
        ...
        print(f"Failed to print analysis summary: {str(e)}")
    ...
    while True:
        try:
            ...
            if choice == "1":
                ...
                search_menu()  # Note: search_menu is not defined in the provided code
            elif choice == "3":
                print("Goodbye!")
                break
            else:
