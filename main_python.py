...
try:
    ...
    try:
        ...
        if os.path.exists(resource_final_path):
            os.rmtree(resource_final_path)
        os.move(os.path.join(resource_extract_path, "resources"), resource_final_path)

        os.remove(resource_zip_path)
        os.rmtree(resource_extract_path)
...
def split_into_sentences(text, language="en"):
    try:
        ...
        # Fallback to regex for both English and Malayalam
        print(f"Using regex sentence splitting for language: {language}")
...
def create_zip_archive(audio_path, raw_transcription, ml_translation, pdf_report, 
    ...
    """Create a zip archive with all analysis files, including a merged summary report."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
