...
class MalayalamTranscriptionPipeline:
    def __init__(self, model_size="base"):
        # Check for GPU availability, fall back to CPU
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"Loading Faster-Whisper {model_size} model on {self.device}...")

        # Configure threading for CPU or compute type for GPU
        if self.device == "cpu":
            # Maximize CPU threads for CPU usage
            num_threads = os.cpu_count() or 1  # Fallback to 1 if cpu_count() returns None
            torch.set_num_threads(num_threads)
            print(f"Configured PyTorch to use {num_threads} CPU threads for maximum performance")
            compute_type = "int8"  # Memory-efficient for CPU
            cpu_threads = num_threads  # Pass to WhisperModel
        else:
            # GPU settings
            print("GPU detected, using CUDA for accelerated processing")
            compute_type = "float16"  # Performance-optimized for GPU
            cpu_threads = 0  # No CPU threads needed for GPU
        ...
        try:
            self.model = WhisperModel(
                ...
                cpu_threads=cpu_threads if self.device == "cpu" else 0  # CPU threads only for CPU
            )
            print(f"Successfully loaded {model_size} model on {self.device}" +
                  (f" with {cpu_threads} CPU threads" if self.device == "cpu" else ""))
        except Exception as e:
        ...
        try:
            ...
            print(f"Error loading translation pipeline: {str(e)}")
            raise

        self.temp_files = []  # Initialize empty list for temporary files
    ...
    def transcribe_audio(self, audio_path):
        try:
            ...
            print("Transcribing audio with Faster-Whisper using parallel processing...")
            num_workers = os.cpu_count() or 1
            print(f"Using {num_workers} workers for transcription")
...
def split_into_sentences(text: str, language: str = "en") -> list[str]:
    ...
    try:
        ...
        # Step 2: Additional splitting on standalone commas or full stops
        final_sentences = []
