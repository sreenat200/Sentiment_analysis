    ...
    try:
        ...
        with tab1:
            ...
            with col1:
                ...
                st.text_area("Translated Text", results["ml_translation"], height=200)
        ...
        with tab3:
            ...
            min_length = min(len(results["en_analysis"]), len(results["ml_analysis"]))
            if min_length > 0:
                df_trend = pd.DataFrame({
                    'Sentence': list(range(1, min_length + 1)),
                    'English': [item["sentiment_score"] for item in results["en_analysis"]][:min_length],
                    'Malayalam': [item["sentiment_score"] for item in results["ml_analysis"]][:min_length]
                })
                fig4 = px.line(df_trend, x='Sentence', y=['English', 'Malayalam'], 
            ...
            if min_length > 0:
                sentiment_diffs = [abs(en["sentiment_score"] - ml["sentiment_score"]) 
                                   for en, ml in zip(results["en_analysis"][:min_length], results["ml_analysis"][:min_length])]
                fig5 = px.histogram(sentiment_diffs, nbins=10, title="English-Malayalam Sentiment Differences")
