...
st.markdown("""<style>
    ...
    .logo-container {
        ...
        gap: 15px;
        margin-bottom: 10px;
        margin-top: -15px; /* Adjusted to move title further upward */
    }
    .glow-logo {
        width: 60px;
        height: 60px;
    ...
    try:
        if files:
            ...
            for idx, file in enumerate(sorted_files[:display_count]):
                ...
                emoji = get_emotion_emoji(emotion)
                # Fix: Use file instead of entry for timestamp
...
def display_dashboard():
    ...
    st.markdown('<h4 style="color:white; margin-top:5px;">Recently Processed Files</h4>', unsafe_allow_html=True)
...
def main():
    try:
        ...
        st.markdown("---")
        st.markdown('<h4 style="color:white; margin-top:5px;">Search Audio From Drive</h4>', unsafe_allow_html=True)
