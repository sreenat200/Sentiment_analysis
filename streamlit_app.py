...
def split_into_sentences(text: str, language: str = "en") -> list:
    """
    Improved sentence splitting function without NLTK or Indic NLP dependencies
    """
    ...
    try:
        if language == "en":
            # English sentence splitting with improved rules
            sentence_endings = re.compile(
                r'(?<!\w\.\w.)'       # Don't match abbreviations like e.g.
                r'(?<![A-Z][a-z]\.)'  # Don't match abbreviations like U.S.
                r'(?<=\.|\?|\!|\。”|\.”)'  # Support for quotes after punctuation
            ...
            # Malayalam-specific sentence splitting
            sentence_endings = re.compile(
                r'(?<=[.!?]|\.\”|\.\"|\。」)'  # Standard punctuation
                r'|(?<=[ാൂീേോൊൗ])'          # Malayalam vowel signs
                r'\s+'                        # Split on whitespace
            )
            sentences = sentence_endings.split(text)
            return [s.strip() for s in sentences if s.strip()]
...
def process_audio(audio_file, model_size, progress_callback=None):
    try:
        ...
        try:
            ...
            # Use our improved sentence splitter
            ...
            # Ensure alignment by padding/truncating
            ...
            st.session_state.processing_error = f"Translation failed: {str(e)}. Pram translation."
        if progress_callback:
        ...
        # Analyze Malayalam translations, aligning with English sentences
        ml_analysis = []
        ...
        results = {
            ...
            "audio_metadata": {
                "emotion": st.session_state.transcriber.analyze_emotion(wav_path) if 'transcriber' in st.session_state else "unknown"
            }
        }
...
def display_results():
    try:
        ...
        # Display emotion at the top
        emotion = results.get('audio_metadata', {}).get('emotion', 'unknown')
        emoji = get_emotion_emoji(emotion)
        st.markdown(f"""
        <div class="emotion-display">
            <h3 class="emotion-text">Detected Emotion: {emotion.capitalize()} {emoji}</h3>
        </div>
        """, unsafe_allow_html=True)
