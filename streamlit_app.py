...
from gdrive_utils import upload_to_gdrive, search_gdrive_files
import tempfile
import pandas as pd
...
def display_search_results(files):
    ...
    if files:
        ...
        for file in files:
            # Extract scores from filename if available
            filename = file['name']
            lead_score = "N/A"
            intent_score = "N/A"
            
            # Try to extract scores from filename pattern
            import re
...
def main():
    ...
    with col1:
        if selected_filter == "Filename":
            ...
            score_col1, score_col2 = st.columns(2)
            with score_col1:
                min_intent = st.number_input("Minimum intent score", min_value=0, max_value=100, value=0, key="min_intent")
            with score_col2:
                max_intent = st.number_input("Maximum intent score", min_value=0, max_value=100, value=100, key="max_intent")
            
            if st.button("Search by Intent Score", key="intent_search_button"):
                with st.spinner("Searching Google Drive..."):
                    files = search_gdrive_files(
            ...
            if st.button("Search by Date Range", key="date_search_button"):
                with st.spinner("Searching Google Drive..."):
                    files = search_gdrive_files(
