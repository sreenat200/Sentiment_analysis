...
def load_storage_config():
    ...
    return {'recent_dirs': [], 'last_selected_dir': default_storage_dir}
...
def display_results():
    try:
        ...
        with tab1:
            ...
            base_lead_score = results.get("lead_score_breakdown", {}).get("base_lead_score", results["lead_score"])
            positive_matches = results.get("lead_score_breakdown", {}).get("positive_matches", [])
            negative_matches = results.get("lead_score_breakdown", {}).get("negative_matches", [])
            total_extra_points = results.get("lead_score_breakdown", {}).get("total_extra_points", 0)
            
            st.markdown(f"""
            <div class="expander-content">
                <strong>Base Lead Score:</strong> {base_lead_score}/100<br>
                <strong>Keyword Adjustments:</strong><br>
            """, unsafe_allow_html=True)
            
            if positive_matches or negative_matches:
                for match in positive_matches:
                    st.markdown(f"Added {match['points']} points for '{match['keyword']}' in {match['language']} sentence {match['sentence_index']}")
                for match in negative_matches:
                    st.markdown(f"Subtracted {abs(match['points'])} points for '{match['keyword']}' in {match['language']} sentence {match['sentence_index']}")
                st.markdown(f"<strong>Total Adjustment:</strong> {total_extra_points:+} points")
            else:
                st.markdown("No keyword-based adjustments applied.")
            st.markdown(f"<strong>Final Lead Score:</strong> {results['lead_score']}/100")
...
def display_dashboard():
    ...
    st.markdown('<h4 style="text-underline-offset: 4px; font-weight: 400;color:var(--text-color);margin-bottom:3px;text-decoration: underline; margin-top:-15px;">Recent files</h4>', unsafe_allow_html=True)
    ...
    if st.session_state.storage_metadata:
        ...
        for entry in st.session_state.storage_metadata:
            ...
            # Fallback to regex parsing if scores are not explicitly stored
        ...
        for idx, entry in enumerate(sorted_metadata[:display_count]):
            ...
            with st.expander(expander_label):
                ...
                # Display lead and intent scores with validation
...
def main():
    try:
        ...
        loading_placeholder = st.empty()  # Placeholder for loading animation
        results_container = st.container()
        dashboard_container = st.container()

        # Render logo and title in the header container
        ...
        # Display results in the results container (only if analysis is complete)
        ...
        # Display dashboard in its own container
        ...
        # Sidebar for audio upload, settings, agent management, and storage
        with st.sidebar:
            ...
            if audio_file:
                ...
                if agents:
                    ...
                    st.warning("No agents created yet. Create an agent in Agent Management.")
                    st.session_state.selected_agent = "NoAgent"
            ...
            model_size = st.selectbox(
                "Model Size",
                ["small", "medium", "large", "large-v1", "large-v2", "large-v3"],
                index=1,
            ...
            # Agent deletion with PIN verification
            agents = load_agent_config()
            if agents:
            ...
            if storage_choice == "Recent Directories" and recent_dirs:
                ...
                if st.button("Use Selected Directory", key="sidebar_use_recent_dir_button"):
                ...
                if st.button("Select Storage Directory", key="sidebar_tkinter_select_dir_button"):
                ...
                # JavaScript for directory picker
                ...
                <script>
                    ...
                    picker.onchange = () => {
                        if (picker.files.length > 0) {
                            ...
                            // Store path in sessionStorage to retrieve in Python
                            sessionStorage.setItem('selectedDirectory', dirPath);
                            // Trigger form submission or reload to process in Streamlit
                ...
                # Check sessionStorage for selected directory
                selected_dir = st.session_state.get('selected_directory', None)
                if selected_dir is None:
                    # JavaScript to retrieve sessionStorage value
                    get_session_storage_js = """
                    <script>
                        const dirPath = sessionStorage.getItem('selectedDirectory');
                        if (dirPath) {
                            // Post message to parent to communicate with Streamlit
                    ...
                    # Handle JavaScript message in Streamlit
                ...
                if selected_dir:
                    if os.path.isdir(selected_dir):
                        if selected_dir != st.session_state.storage_dir:
                            ...
                            update_recent_dirs(st.session_state.storage_dir)
                            st.success(f"Storage directory set to: {st.session_state.storage_dir}")
                            # Clear sessionStorage to avoid reprocessing
        ...
        # Process uploaded audio with loading animation
        if audio_file and st.session_state.get('process_triggered') and not st.session_state.get('analysis_complete'):
            with loading_placeholder.container():  # Use the placeholder below the title
                ...
                def update_progress(progress, status):
                    try:
                        ...
                        import time
                        time.sleep(0.1)  # Ensure rendering
                    except Exception as e:
        ...
        # Process stored audio from dashboard
        if st.session_state.get('selected_audio') and st.session_state.get('process_triggered') and not st.session_state.get('analysis_complete'):
            with loading_placeholder.container():  # Use the same placeholder
                ...
                with st.spinner("Analyzing stored audio... Please wait."):
                    try:
