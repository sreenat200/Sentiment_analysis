...
def initialize_session_state():
    defaults = {
        ...
        'selected_audio': None,
        'storage_metadata': [],
        'storage_dir': os.path.join(os.getcwd(), 'lead_scoring_storage'),  # Default storage directory
...
def main():
    try:
        ...
        with st.sidebar:
            ...
            if st.button("Select Storage Directory"):
                folder = select_folder()
                if folder and os.path.isdir(folder):
                    st.session_state.storage_dir = folder
                    os.makedirs(folder, exist_ok=True)
                    initialize_session_state()
                    st.success(f"Storage directory set to: {folder}")
                    st.rerun()
                else:
            ...
            if manual_dir and os.path.isdir(manual_dir):
                ...
                initialize_session_state()
                st.success(f"Storage directory set to: {manual_dir}")
                st.rerun()
