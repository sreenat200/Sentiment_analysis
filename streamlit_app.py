...
def initialize_session_state():
    ...
    defaults = {
        ...
        'selected_audio': None,
        'storage_metadata': [],
        'storage_dir': os.path.join(base_storage_path, 'default'),  # Default storage directory
...
def main():
    try:
        ...
        with st.sidebar:
            ...
            st.markdown("---")
            st.header("Settings")
            model_size = st.selectbox("Model Size", ["small", "medium", "large", "large-v1", "large-v2", "large-v3"], index=1)
            ...
            storage_choice = st.selectbox("Select Storage Location", storage_options, key="storage_choice")
            base_storage_path = os.path.join(os.getcwd(), 'app_storage', 'user_dirs')
            if storage_choice == "Default Storage":
                st.session_state.storage_dir = os.path.join(base_storage_path, 'default')
                os.makedirs(st.session_state.storage_dir, exist_ok=True)
                initialize_session_state()
                st.success(f"Storage directory set to: {st.session_state.storage_dir}")
            elif storage_choice == "Custom Directory":
                custom_dir_name = st.text_input("Enter Directory Name (e.g., my_storage)", key="custom_storage_dir")
                if custom_dir_name:
                    # Sanitize directory name to prevent invalid characters
                    custom_dir_name = "".join(c for c in custom_dir_name if c.isalnum() or c in ('-', '_')).rstrip()
                    if custom_dir_name:
                        st.session_state.storage_dir = os.path.join(base_storage_path, custom_dir_name)
                        os.makedirs(st.session_state.storage_dir, exist_ok=True)
                        initialize_session_state()
                        st.success(f"Storage directory set to: {st.session_state.storage_dir}")
                    else:
                ...
                if st.button("Select Storage Directory", key="tkinter_select_dir"):
                    ...
                    if folder and os.path.isdir(folder):
                        ...
                        st.error("Invalid or no directory selected. Please try again or enter manually.")
                manual_dir = st.text_input("Or enter directory path manually", key="manual_storage_dir")
                if manual_dir and os.path.isdir(manual_dir):
                    ...
                    st.success(f"Storage directory set to: {st.session_state.storage_dir}")
                    st.rerun()
                elif manual_dir and not os.path.isdir(manual_dir):
